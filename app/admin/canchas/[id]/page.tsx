...
export default function EditarCanchaPage() {
    ...
    const [existingFotos, setExistingFotos] = useState<Array<{ url: string }>>([]);
    ...
    const fetchCancha = async () => {
        const res = await fetch(`/api/canchas/${params.id}`);
        const data = await res.json();
        ...
        setFormData({
            ...
            aceptaEventos: data.aceptaEventos || false,
            aceptaFutbol: data.aceptaFutbol || false,
            aceptaVoley: data.aceptaVoley || false,
            tipoCancha: data.tipoCancha || '6v6',
            ...
            diasAtencion: data.diasAtencion || 'Lunes a Domingo'
        });
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            if (res.ok) {
                ...
                await fetch(`/api/fotos/${params.id}`, {method: 'DELETE'});

                const allUrls = [...existingFotos.map(f => f.url)];
                ...
                if (fotosFiles.length > 0) {
                    const uploadedUrls = await Promise.all(
                        fotosFiles.map(async (file) => {
                            ...
                            formData.append('canchaId', params.id as string);
                    ...
                    allUrls.push(...uploadedUrls.filter(url => url !== null));
                }
    ...
    return (
        <div className="container mx-auto px-4 py-8 max-w-4xl">
            <h1 className="text-3xl font-bold mb-8">Editar Cancha</h1>
            ...
            <form onSubmit={handleSubmit} className="card space-y-6">
                    ...
                    <div className="grid md:grid-cols-2 gap-4 mb-4">
                        ...
                        <div>
                            ...
                            <select name="distritoId" value={formData.distritoId} onChange={handleChange}
                                    className="input-field" required disabled={!formData.ciudadId}>
                ...
                <div className="border-b pb-6">
                    ...
                    <div className="mb-4">
                        ...
                        <input type="url" name="googleMapsUrl" value={formData.googleMapsUrl} onChange={handleChange}
                    ...
                    <div className="grid md:grid-cols-2 gap-4">
                        <div>
                            ...
                            <input type="number" step="any" name="latitud" value={formData.latitud}
                                   onChange={handleChange} className="input-field"/>
                        </div>
                        <div>
                            <label className="block text-sm font-medium mb-2">Longitud</label>
                            <input type="number" step="any" name="longitud" value={formData.longitud}
                                   onChange={handleChange} className="input-field"/>
                        </div>
                    </div>
                </div>
                ...
                <div className="border-b pb-6">
                    ...
                    <div className="grid md:grid-cols-3 gap-4">
                        <div>
                            ...
                            <input type="tel" name="telefono1" value={formData.telefono1} onChange={handleChange}
                                   className="input-field"/>
                        </div>
                        <div>
                            <label className="block text-sm font-medium mb-2">Teléfono 2</label>
                            <input type="tel" name="telefono2" value={formData.telefono2} onChange={handleChange}
                                   className="input-field"/>
                        </div>
                        <div>
                            <label className="block text-sm font-medium mb-2">Teléfono 3</label>
                            <input type="tel" name="telefono3" value={formData.telefono3} onChange={handleChange}
                                   className="input-field"/>
                        </div>
                    </div>
                </div>
                ...
                <div className="border-b pb-6">
                    ...
                    <div className="mb-4">
                        ...
                        <select name="diasAtencion" value={formData.diasAtencion} onChange={handleChange}
                                className="input-field">
                    ...
                    <div className="grid md:grid-cols-2 gap-4">
                        <div>
                            ...
                            <input type="time" name="horarioInicio" value={formData.horarioInicio}
                                   onChange={handleChange} className="input-field"/>
                        </div>
                        <div>
                            <label className="block text-sm font-medium mb-2">Hora de Cierre</label>
                            <input type="time" name="horarioFin" value={formData.horarioFin} onChange={handleChange}
                                   className="input-field"/>
                        </div>
                    </div>
                </div>
                ...
                <div className="border-b pb-6">
                    ...
                    <div className="space-y-3">
                        <label className="flex items-center gap-2">
                            <input type="checkbox" name="techada" checked={formData.techada} onChange={handleChange}
                                   className="w-4 h-4"/>
                            <span className="text-sm font-medium">Cancha techada</span>
                        </label>
                        <label className="flex items-center gap-2">
                            <input type="checkbox" name="aceptaFutbol" checked={formData.aceptaFutbol}
                                   onChange={handleChange} className="w-4 h-4"/>
                            <span className="text-sm font-medium">Acepta fútbol</span>
                        </label>
                        <label className="flex items-center gap-2">
                            <input type="checkbox" name="aceptaVoley" checked={formData.aceptaVoley}
                                   onChange={handleChange} className="w-4 h-4"/>
                            <span className="text-sm font-medium">Acepta vóley</span>
                        </label>
                        <label className="flex items-center gap-2">
                            <input type="checkbox" name="aceptaEventos" checked={formData.aceptaEventos}
                                   onChange={handleChange} className="w-4 h-4"/>
                ...
                <div className="pb-6">
                    ...
                    {existingFotos.length > 0 && (
                        <div className="mb-4">
                            <h3 className="text-sm font-medium mb-2">Fotos actuales</h3>
                            <div className="grid grid-cols-3 gap-4">
                                {existingFotos.map((foto, index) => (
                                    <div key={index} className="relative group">
                                        <img src={foto.url} alt={`Foto ${index + 1}`}
                                             className="w-full h-32 object-cover rounded-lg"/>
                                        <button type="button" onClick={() => removeExistingFoto(index)}
                                                className="absolute top-2 right-2 bg-red-500 text-white rounded-full w-6 h-6 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity">×
                                        </button>
                                        {index === 0 && <span
                                            className="absolute bottom-2 left-2 bg-primary-500 text-white text-xs px-2 py-1 rounded">Principal</span>}
                                    </div>
                                ))}
                            </div>
                        </div>
                    ...
                    <div className="mb-4">
                        <label className="block">
                            <div
                                ...
                                <span className="text-sm text-gray-600">Click para agregar más imágenes</span>
                    ...
                    {fotosFiles.length > 0 && (
                        <div>
                            <h3 className="text-sm font-medium mb-2">Nuevas fotos a subir</h3>
                            <div className="grid grid-cols-3 gap-4">
                ...
                <div className="flex gap-4">
                    <button type="submit" className="btn-primary flex-1" disabled={uploading}>
                        {uploading ? 'Actualizando...' : 'Actualizar Cancha'}
                    </button>
